...
const tiles = 16; //in pixels
const plots = tiles * 9; //in pixels
const roads = tiles * 0; //in pixels
const initialOffsets = plots +roads;
const plotViewOffsets = plots +roads;
...
function validateMove(direction) {
    switch(direction){
        case 'ArrowRight': return !(plotView.plotX == 600);
        case 'ArrowUp': return   !(plotView.plotY == -100);
        case 'ArrowLeft': return !(plotView.plotX == -100);
        case 'ArrowDown': return !(plotView.plotY == 600);
    }
}
...
async function assignPlot() {
    ...
    if (!assigned) {
        const metadata = {
            ...
            "image":"https://gateway.pinata.cloud/ipfs/QmYt3X7SpJVkHyRW4Sa4r9Rj2933XogcB367PpP3aQse1P",
        }
